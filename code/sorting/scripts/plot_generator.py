...
# Lee los CSV de data/measurements/ y genera PNG en data/plots/.
# Referencias:
...
BASE = pathlib.Path(__file__).resolve().parents[1]   # code/sorting
...
agg = (df.groupby(["algorithm", "type", "n"])
         .agg(time_ms=("time_ms", "mean"), mem_kb=("mem_kb", "mean"))
         .reset_index())
...
for metric, ylabel, fname in [("time_ms", "Tiempo (ms)", "tiempos"),
    ...
    for ax, t in zip(axes, TIPOS):
        ...
        for alg, g in sub.groupby("algorithm"):
            ...
            if metric == "time_ms":
                ax.set_yscale("log")
        ax.set_title(f"Entrada {t}")
        ax.set_xlabel("n")
        ax.grid(True, which="both", ls=":", alpha=0.6)
    axes[0].set_ylabel(ylabel)
    axes[-1].legend(fontsize=9)
    fig.suptitle(f"Ordenamiento: {ylabel} vs n (promedio sobre dominios y muestras)")
    fig.tight_layout()
